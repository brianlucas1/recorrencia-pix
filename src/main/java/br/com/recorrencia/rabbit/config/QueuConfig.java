...
public class QueuConfig {
    ...
    public Declarables declaracoesFilasPix(
        ...
        TopicExchange scheduleExchange   = new TopicExchange(scheduleExchangeName, true, false);
        ...
        Binding scheduleBinding = BindingBuilder.bind(scheduleQueue)
                .to(scheduleExchange).with(scheduleRoutingKey);

        Binding retryBinding = BindingBuilder.bind(retryQueue)
                .to(retryExchange).with(retryRoutingKey);

        Binding deadLetterBinding = BindingBuilder.bind(deadLetterQueue)
                .to(deadLetterExchange).with(deadLetterRoutingKey);

        return new Declarables(
                scheduleExchange, retryExchange, deadLetterExchange,
                scheduleQueue, retryQueue, deadLetterQueue,
                scheduleBinding, retryBinding, deadLetterBinding
        );
    }
}
